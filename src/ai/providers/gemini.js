...
// baseUrl lets a proxy or gateway stand in for the public endpoint.
const root = (baseUrl) => `${(baseUrl || DEFAULT_ROOT).replace(/\/+$/, '')}/models`;
...
export default {
    ...
    loosePattern: /^AIza[0-9A-Za-z_-]{20,}$/,
    ...
    async listModels({apiKey, baseUrl} = {}) {
        const payload = await getJson(root(baseUrl), {
    ...
    async generate({prompt, apiKey, model, baseUrl}) {
        ...
        try {
            const payload = await postJson(`${root(baseUrl)}/${name}:generateContent`, {
                headers: {'x-goog-api-key': apiKey},
